    ...
    getUserConfig = ({ configPath }) => {
      ...
      if (!import_fs.default.existsSync(userConfigPath)) {
        import_fs.default.writeFileSync(
          userConfigPath,
          JSON.stringify(defaultConfig_default, null, 2),
          "utf-8"
        );
      }
...
var import_prompts2, import_colors, import_fs2, initialise, initialise_default;
var init_initialise = __esm({
  "apps/cli/src/commands/config/initialise.ts"() {
    init_defaultConfig();
    import_prompts2 = require("@clack/prompts");
    import_colors = __toESM(require("colors"), 1);
    ...
    initialise = async ({ command }) => {
      ...
      if (force || !import_fs2.default.existsSync(configFilePath)) {
        import_prompts2.log.info(`Creating configuration file '${import_colors.default.cyan(configFilePath)}'`);
        import_fs2.default.writeFileSync(configFilePath, JSON.stringify(defaultConfig_default, null, 2));
      }
    };
...
var import_fs3, import_cloneDeep2, import_path2, booleanParse, set, set_default;
var init_set = __esm({
  "apps/cli/src/commands/config/set.ts"() {
    init_defaultConfig();
    init_mergeConfigs();
    ...
    set = async ({ program, context, command }) => {
...
var init_on = __esm({
  "apps/cli/src/commands/debug/on.ts"() {
    ...
    debugOn = async ({ context }) => {
      console.log("Hi");
      context.addCommand(new CommandInstance_default(["set", "debug", "true"]));
    };
    on_default = debugOn;
  }
});
...
var import_colors2, formatCommand, formatCommand_default;
var init_formatCommand = __esm({
  "libs/utils/src/helpers/format/formatCommand.ts"() {
    import_colors2 = __toESM(require("colors"));
    formatCommand = (command) => import_colors2.default.yellow(command);
    formatCommand_default = formatCommand;
  }
});

// libs/utils/src/helpers/format/formatNumber.ts
var import_colors3, formatNumber, formatNumber_default;
var init_formatNumber = __esm({
  "libs/utils/src/helpers/format/formatNumber.ts"() {
    import_colors3 = __toESM(require("colors"));
    formatNumber = (number) => import_colors3.default.cyan(String(number));
...
var import_prompts4, import_colors4, commit, commit_default;
  ...
  "apps/cli/src/commands/git/commit.ts"() {
    ...
    commit = async ({ command }) => {
      ...
      if (stagedFiles.length === 0) {
        ...
        const unstagedMessage = unstagedLen > 0 ? ` ${import_colors4.default.yellow(String(unstagedLen))} Unstaged files detected.` : "";
      ...
      const flowDetectedMessage = isFlow ? ` (${import_colors4.default.cyan("Flow")})` : "";
      const messageTitle = `Enter commit message for branch '${import_colors4.default.yellow(name)}'${flowDetectedMessage}:`;
...
var import_prompts7, import_colors5, push, push_default;
var init_push = __esm({
  "apps/cli/src/commands/git/push.ts"() {
    init_execAsync();
    init_formatCommitsList();
    ...
    import_colors5 = __toESM(require("colors"), 1);
    push = async () => {
      const { name: branchName } = await getBranchDescription_default();
      const commits = await getLocalCommits_default({ name: branchName });
      if (commits.length === 0) {
        import_prompts7.log.info(`No commits to push at branch '${import_colors5.default.yellow(branchName)}'`);
        return;
      }
...
var ProgramInstance = class {
  constructor(argv) {
    this.preffix = import_colors6.default.cyan("\u2325\u2387 cli \uE0B0");
  ...
  get log() {
    if (!this._log)
      this._log = new LoggerInstance_default({
        enabled: true,
        preffix: import_colors6.default.cyan("[AltZap:cli]:")
      });
    return this._log;
  }
  get debug() {
    if (!this._debug)
      this._debug = new LoggerInstance_default({
        ...
        preffix: import_colors6.default.magenta("\u2325\u2387:cli \uE0B0 \u{1F6A7}")
      });
    return this._debug;
  }
};
